...
// query users
$stmt = $user->searchPaging($keywords, $from_record_num, $records_per_page);
$num = $stmt->rowCount();
...
if ($num > 0) {
	...
	// include paging
	$total_rows = $user->countSearch($keywords);
	$page_url = "{$home_url}user/search_paging.php?s={$keywords}&";
